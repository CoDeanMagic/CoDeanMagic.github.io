...
function getTravelRecommendation(e) {
  ...
  const stationLocationEl = stationElement.querySelector(".station-location");
  const stationName = stationNameEl.innerText.split("：")[1];
  const stationLocation = stationLocationEl.innerText.split("：")[1];
  const travelInfoContent = document.querySelector(
    "#travel-recommendation-info"
  );

  travelInfoContent.innerHTML = `<div class="d-flex justify-content-center">
  ...
  myHeaders.append("Authorization", "Bearer app-707hxjRMfqDWUspKW0lxR188");
  ...
  //   .then((result) => console.log(result.data.outputs.text))
  ...
  fetch("https://agent.build-school.com/v1/workflows/run", requestOptions)
    ...
    .then((result) => {
      travelInfoContent.innerHTML = marked.parse(result.data.outputs.text);
    })
    .catch((error) => {
